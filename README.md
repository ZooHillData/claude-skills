...
| **save-output** | `/save-output [description of which content]` | Save a long report or generated content from the conversation to a markdown file in ~/.scratch/claude/. Use when the user asks to "save that output", "export the report", "write that to a file", or uses /save-output. |
| **save-plan** | `/save-plan <slug> [content-source]` | Save the current plan or conversation context to a dated plan file. This skill should be used when the user asks to "save this plan", "archive the plan", "write the plan to a file", or uses /save-plan. |
